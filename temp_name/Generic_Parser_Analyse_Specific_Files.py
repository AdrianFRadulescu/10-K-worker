...
from database_api import get_files_by_ciks, is_file_in_database
...
# User defined file containing the ciks for the files that need to be analyzed
# If all the files need to be analysed then replace path with "all"
TARGET_FILES_CIKS = '/Users/adrian_radulescu1997/Documents/Uni-Courses/DBPartTimeJob/crawler/temp_name/ciks'

if TARGET_FILES_CIKS is 'all':
    TARGET_FILES = get_all_files(path=FILE_SYSTEM_PATH,
                                 sorted=True)  # returns all files from the folder given and sub-folders
else:
    TARGET_FILES = get_files_by_ciks(TARGET_FILES_CIKS)

#TARGET_FILES = get_files_by_ciks(TARGET_FILES_CIK_FOLDER)


#
# TARGET_FILES = execute_SQL_select_query('');

# True if adding to database, false otherwhise
